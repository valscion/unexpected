    ...
    expect.addType({
        ...
        inspect: function (arr, depth, output, inspect) {
            ...
            var inspectedItems = keys.map(function (key) {
                if (key in arr) {
                    return output.clone().property(key, inspect(arr[key]), true);
                } else {
                    return output.clone();
                }
            });
                    ...
                    if (type === 'insert') {
                        ...
                        this.block(function () {
                            ...
                            if (valueDiff && valueDiff.inline) {
                                inspectedValue = output.clone().block(valueDiff.diff.amend(delimiterOutput));
                            } else if (valueDiff) {
                                inspectedValue = output.clone().block(inspect(diffItem.value).amend(delimiterOutput.sp())).annotationBlock(function () {
                            ...
                            this.property(diffItem.actualIndex, inspectedValue, true);
                        });
                    }
                });
            });

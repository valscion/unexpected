    ...
    expect.addType({
        ...
        inspect: function (arr, depth, output, inspect) {
            ...
            var inspectedItems = keys.map(function (key) {
                if (key in arr) {
                    return output.clone().key(key, true).nonLeadingSpace().append(inspect(arr[key]));
                } else {
                    return output.clone();
                }
            });
                    ...
                    if (type === 'insert') {
                        this.annotationBlock(function () {
                            this.error('missing ').block(function () {
                                this.key(typeof diffItem.actualIndex !== 'undefined' ? diffItem.actualIndex : diffItem.expectedIndex, true)
                                    .nonLeadingSpace()
                        ...
                        this.block(function () {
                            this.key(diffItem.actualIndex, true)
                                .nonLeadingSpace()
                                .append(inspect(diffItem.value))

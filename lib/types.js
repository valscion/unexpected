    ...
    expect.addType({
        ...
        inspect: function (arr, depth, output, inspect) {
            ...
            var inspectedItems = keys.map(function (key) {
                if (key in arr) {
                    return output.clone().arrayKey(key).nonLeadingSpace().appendInspected(arr[key]);
                } else {
                    return output.clone();
                }
            });
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            if (!actualType.numericalPropertiesOnly || !actualType.numericalPropertiesOnly) {

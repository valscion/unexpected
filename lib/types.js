        ...
        inspect: function (obj, depth, output, inspect) {
            ...
            var inspectedItems = keys.map(function (key, index) {
                ...
                if (inspectedValue.isBlock() && inspectedValue.isMultiline()) {
                    ...
                    propertyOutput.nonLeadingSpace();
                }
    ...
    expect.addType({
        ...
        equal: function (a, b, equal) {
            if (a === b) {
                ...
                var aType = expect.findTypeOf(a);
                var bType = expect.findTypeOf(b);
                var i;
                if (aType.numericalPropertiesOnly && bType.numericalPropertiesOnly) {
                    ...
                    var aKeys = aType.getKeys(a);
                    var bKeys = bType.getKeys(b);
        ...
        inspect: function (arr, depth, output, inspect) {
            ...
            var inspectedItems = keys.map(function (key) {
                if (key in arr) {
                    return output.clone().arrayKey(key).nonLeadingSpace().append(inspect(arr[key]));
                } else {
                    return output.clone();
                }
            });
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            var actualType = expect.findTypeOf(actual);
            var expectedType = expect.findTypeOf(expected);
            if (!actualType.numericalPropertiesOnly || !expectedType.numericalPropertiesOnly) {
                var isSeenByNonNumericalKey = {};
                actualType.getKeys(actual).forEach(function (key) {
                    if (!numericalRegExp.test(key)) {
                        isSeenByNonNumericalKey[key] = true;
                    }
                });
                expectedType.getKeys(expected).forEach(function (key) {
                    if (!numericalRegExp.test(key)) {
                        isSeenByNonNumericalKey[key] = true;
                    }
                });
                Object.keys(isSeenByNonNumericalKey).forEach(function (key) {
                    if (key in actual) {
                        if (key in expected) {
                            changes.push({
                    ...
                    if (type === 'insert') {
                        this.annotationBlock(function () {
                            this.error('missing ').block(function () {
                                this.arrayKey(typeof diffItem.actualIndex !== 'undefined' ? diffItem.actualIndex : diffItem.expectedIndex)
                                    .nonLeadingSpace()
                                    .append(inspect(diffItem.value));
                            });
                        });
                    } else if (type === 'remove') {
                        this.block(function () {
                            this.arrayKey(diffItem.actualIndex)
                                .nonLeadingSpace()
                                .append(inspect(diffItem.value))

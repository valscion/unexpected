                        ...
                        subjectKeys.forEach(function (key, index) {
                            output.i().block(function () {
                                if (subjectIsArrayLike) {
                                    ...
                                    this.key(key).nonLeadingSpace();
                                }
                                this.append(inspect(subject[key]));
        ...
        return expect.promise.all([
            ...
            return expect.promise.settle(keyPromises).then(function () {
                ...
                function failWithChanges(changes) {
                    if (!subjectType.numericalPropertiesOnly || !valueType.numericalPropertiesOnly) {
                        ...
                        Object.keys(isSeenByNonNumericalKey).forEach(function (key) {
                            if (key in subject) {
                                if (key in value) {
                                    changes.push({
                                        type: keyPromises[key].isFulfilled() ? 'equal' : 'similar',
                                        actualIndex: key,
                                        expectedIndex: key,
                                        value: subject[key],
                                        expected: value[key]
                                    ...
                                    if (type === 'insert') {
                                        this.annotationBlock(function () {
                                            if (expect.findTypeOf(diffItem.value).is('function')) {
                                                this.error('missing: ')
                                                    .arrayKey(typeof diffItem.actualIndex !== 'undefined' ? diffItem.actualIndex : diffItem.expectedIndex)
                                                    .nonLeadingSpace()
                                                    .block(function () {
                                        ...
                                        if (typeof diffItem.actualIndex === 'string') {
                                            this.arrayKey(diffItem.actualIndex).nonLeadingSpace();
                                        }
                                        if (type === 'remove') {

...
var numericalRegExp = require('./numericalRegExp');
                    ...
                    diff: !expect.flags.not && function (output, diff, inspect, equal) {
                        ...
                        subjectKeys.forEach(function (key, index) {
                            output.i().block(function () {
                                this.key(key, subjectIsArrayLike)
                                    .nonLeadingSpace()
                                    .append(inspect(subject[key]));
        ...
        return expect.promise.all([
            ...
            return expect.promise.settle(keyPromises).then(function () {
                ...
                var changes = arrayChanges(subject, value, function equal(a, b, aIndex, bIndex) {
                    var isNumerical = typeof aIndex === 'number' && typeof bIndex === 'number';
                    if (isNumerical) {
                    ...
                    } else if (keyPromises[aIndex]) {
                        return keyPromises[aIndex].isFulfilled();
                    }
                    var result;
                    try {
                ...
                }, !subjectType.numericalPropertiesOnly || !valueType.numericalPropertiesOnly);
                if (isAsync) {
                    return expect.promise(function (resolve, reject) {
                        arrayChangesAsync(subject, value, function equal(a, b, aIndex, bIndex, cb) {
                            ...
                            if (isNumerical) {
                                ...
                                return cb(keyPromises[aIndex].isFulfilled());
                            }
                            expect.promise(function () {
                        ...
                        }, !subjectType.numericalPropertiesOnly || !valueType.numericalPropertiesOnly, resolve);
                                    ...
                                    if (type === 'insert') {
                                        this.annotationBlock(function () {
                                            if (expect.findTypeOf(diffItem.value).is('function')) {
                                                this.error('missing: ')
                                                    .key(typeof diffItem.actualIndex !== 'undefined' ? diffItem.actualIndex : diffItem.expectedIndex, true)
                                                    .nonLeadingSpace()
                                                    .block(function () {
                                                ...
                                                this.error('missing ')
                                                    .key(typeof diffItem.actualIndex !== 'undefined' ? diffItem.actualIndex : diffItem.expectedIndex, true)
                                                    .nonLeadingSpace()
                                                    .block(inspect(diffItem.value));
                                            }
                                        });
                                    } else {
                                        if (typeof diffItem.actualIndex === 'string') {
                                            this.key(diffItem.actualIndex, true).nonLeadingSpace();
                                        }
                                        if (type === 'remove') {
                                            ...
                                            var toSatisfyResult;
                                            if (numericalRegExp.test(diffItem.actualIndex)) {
                                                toSatisfyResult = toSatisfyMatrix[diffItem.actualIndex + subject.length * diffItem.expectedIndex];
                                            } else {
                                                toSatisfyResult = keyPromises[diffItem.actualIndex].reason();
                                            }

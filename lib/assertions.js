...
module.exports = function (expect) {
    ...
    expect.addAssertion('<array-like> to [exhaustively] satisfy <array-like>', function (expect, subject, value) {
        expect.errorMode = 'bubble';
        var i;
        var valueKeys = expect.argTypes[0].getKeys(value);
        var keyPromises = {};
        ...
        return expect.promise.all([
            ...
            return expect.promise.settle(keyPromises).then(function () {
                var toSatisfyMatrix = new Array(subjectKeys.length * valueKeys.length);
                subjectKeys.forEach(function (subjectKey, i) {
                    if (subjectKey in value) {
                        toSatisfyMatrix[i + i * subjectKeys.length] = keyPromises[subjectKey].isFulfilled() || keyPromises[subjectKey].reason();
                    }
                });
                if (subjectKeys.length > 10 || valueKeys.length > 10) {
                    var indexByIndexChanges = [];
                    subjectKeys.forEach(function (subjectKey, i) {
                        var promise = keyPromises[subjectKey];
                        if (i < value.length) {
                            indexByIndexChanges.push({
                                type: promise.isFulfilled() ? 'equal' : 'similar',
                                value: subject[subjectKey],
                                expected: value[subjectKey],
                                actualIndex: i, // ?
                                expectedIndex: i, // ?
                                last: i === Math.max(subjectKeys.length, valueKeys.length) - 1
                            });
                        } else {
                            indexByIndexChanges.push({
                                type: 'remove',

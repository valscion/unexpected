...
module.exports = function (expect) {
    ...
    expect.addStyle('key', function (key, isArrayLike) {
        var isSymbol;
        /*jshint ignore:start*/
    ...
    expect.addStyle('nonLeadingSpace', function (num) {
        var lastLine = this.output[this.output.length - 1];
        if (lastLine.length > 0 && !/\s$/.test(lastLine[lastLine.length - 1].args.content)) {
            this.sp(num);
        }
    });
